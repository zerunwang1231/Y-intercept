...
if output is positive, ie, fast volume average is larger than the slow one,
then it signals a strong trend
...
def signal(vo, bolu, bold, price, window):  # we assume vo, bolu, bold have the same length
    ...
    indicator = [0] * len(
        vo
    )  # 1 means buy, -1 means sell, 0 means do nothing. Short sell is allowed
